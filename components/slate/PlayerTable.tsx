...
const COLUMNS: ColDef[] = [
  ...
  { key: 'Team',     label: 'Team',    tooltip: 'Team abbreviation.',                                                              sortable: true,  mobileHide: true },
  { key: 'OPP',      label: 'OPP',     tooltip: 'Opponent abbreviation.',                                                          sortable: true,  mobileHide: true },
  { key: 'Salary',   label: 'Salary',  tooltip: 'DraftKings salary.',                                                              align: 'right',  sortable: true,  mobileHide: true },
  { key: 'Own',      label: 'Own%',    tooltip: 'Projected ownership percentage. Ownership combined with usage is the strongest predictor of outperformance in backtesting.', align: 'right', sortable: true, mobileHide: true },
  { key: 'Usage',    label: 'Usg%',    tooltip: 'Projected usage rate — the percentage of team plays a player is involved in while on the floor. Key threshold: ≥22%, with position-specific ceilings (Centers must be under 28%).', align: 'right', sortable: true, mobileHide: true },
  { key: 'Proj',     label: 'Proj',    tooltip: 'Model projected fantasy points for this slate.',                                  align: 'right',  sortable: true },
  { key: 'Ceiling',  label: 'Ceil',    tooltip: 'Upside ceiling projection — the score this player could achieve in an optimal game.',  align: 'right', sortable: true, mobileHide: true },
  { key: 'ceil_gap', label: 'Ceiling Gap', tooltip: 'Ceiling Gap = Ceiling minus Projection. Measures available upside. The 16–20 pt gap range correlated with highest overperformance rates (31%). Gaps above 20 may reflect inflated ceilings.', align: 'right', sortable: true, mobileHide: true },
  { key: 'Floor',    label: 'Floor',   tooltip: 'Downside floor projection — the score this player is likely to exceed even in a poor game.',  align: 'right', sortable: true, mobileHide: true },
];
...
const SortHeader: React.FC<{
  ...
  return (
    <th
      ...
      className={`${col.align === 'right' ? 'text-right' : 'text-left'} py-2 px-2 text-[9px] font-black uppercase tracking-widest text-ink/50 select-none whitespace-nowrap ${col.sortable !== false ? 'cursor-pointer hover:text-ink' : ''} ${active ? 'text-ink' : ''} ${col.mobileHide ? 'hidden sm:table-cell' : ''}`}
    >
      ...
      <table className="w-full border-collapse">
        <thead className="border-b border-ink/10 bg-white/60 sticky top-0">
          <tr>
            ...
            {showActuals && (
              <SortHeader
                col={{ key: 'Actual', label: 'Actual', tooltip: 'Actual fantasy points scored.', align: 'right', sortable: true, mobileHide: true }}
                sortCol={sortCol}
                sortDir={sortDir}
                onSort={handleSort}
              />
            ...
            <SortHeader
              ...
              onSort={handleSort}
            />
            <th className="hidden sm:table-cell text-left py-2 px-2 text-[9px] font-black uppercase tracking-widest text-ink/50 whitespace-nowrap">
              Why
            </th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((p, i) => (
            ...
            >
              ...
              {/* Team */}
              <td className="hidden sm:table-cell py-2 px-2 text-[11px] font-black uppercase tracking-widest text-ink/70">
                {p.Team}
              </td>

              {/* OPP */}
              <td className="hidden sm:table-cell py-2 px-2 text-[11px] text-ink/40">
                {p.OPP}
              </td>

              {/* Salary */}
              <td className="hidden sm:table-cell py-2 px-2 text-right text-[11px] font-mono text-ink/70">
                ${p.Salary.toLocaleString()}
              </td>

              {/* Own% */}
              <td className={`hidden sm:table-cell py-2 px-2 text-right text-[11px] font-mono ${ownColor(p.Own)}`}>
                {p.Own.toFixed(1)}%
              </td>
              ...
              {/* Floor */}
              <td className="hidden sm:table-cell py-2 px-2 text-right text-[11px] font-mono text-ink/50">
                {p.Floor.toFixed(1)}
              </td>

              {/* Actuals (historical only) */}
              {showActuals && (
                <td className="hidden sm:table-cell py-2 px-2 text-right text-[11px] font-mono">
                  {p.Actual != null
